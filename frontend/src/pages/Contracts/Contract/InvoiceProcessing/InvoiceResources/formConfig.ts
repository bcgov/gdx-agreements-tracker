...
export const formConfig = (query: UseQueryResult<AxiosResponse, unknown>) => {
  const readFields = !query
    ...
    : [
      ...
      { width: "half", title: "Amount", value: query?.data?.data?.data?.rate * query?.data?.data?.data?.unit_amount },
      { width: "half", title: "Amount Remaining", value: query?.data?.data?.data?.amount_remaining },
    ];
