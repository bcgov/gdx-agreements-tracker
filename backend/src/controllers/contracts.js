...
const getAll = async (request, reply) => {
  if (userCan(request, "contracts_read_all")) {
    try {
    ...
    log.trace('user lacks capability "contracts_read_all"');
...
const getOne = async (request, reply) => {
  if (
    userCan(request, "contracts_read_all") ||
    (userCan(request, "contracts_read_mine") && checkMine(request))
  ) {
    ...
    log.trace('user lacks capability "contracts_read_all" || "contracts_read_mine"');
    return notAllowed(reply);
  }
};
